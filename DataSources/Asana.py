...
class AsanaAPI(object):
    ...
    def __init__(self, apikey="", debug=False):
        ...
        self.apikey = "3D31plGb.tYIr08vhXQI3GEE8XgGm3AG"
        ...
        self.renderers=["ColorBar"]
        random.seed(time.time())


    def getAllProjects(self):
        myspaceid = self.list_workspaces()[0]["id"]  #Result: [{u'id': 123456789, u'name': u'asanapy'}]
        all_projects = self.list_projects(myspaceid,include_archived=False)  #Result: [
        return all_projects

    def chooseProject(self):
        all_projects=self.getAllProjects()
        if not self.options["Randomize"] and len(self.options["Projects"])>0:
            print "not"
            return random.choice([self.getProjectByName(name) for name in self.options['Projects']])
        project=random.choice(all_projects)
        i=0
        while (len(self.get_project_tasks(project["id"]))<self.options["Task_Threshold"]) and i<40:
            project=random.choice(all_projects)
            i+=1
